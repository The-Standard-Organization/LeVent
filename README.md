...
## How it Works
It's essential to understand the ideas around the [Cul-De-Sac pattern](https://www.youtube.com/watch?v=Wgz5m0MY9Xo&ab_channel=HassanHabib) for you to take the most advantage of LeVent.
 
Assume that you have multiple Foundation Services that want to be notified when an event of any type occurs. You can create a simple `EventBroker` that leverages LeVent to offer the registration and publishing capabilities as follows:
...
The next step would be for Orchestration services to publish or listen to the event and call its dependencies.
...
The `StudentLibraryOrchestrationService` will receive a `Student` event from whoever is publishing these events with no coupling and acts upon that very same event by creating a `LibraryCard`.
...
The figure above assumes no dependency between one listener and the other. You can also leverage LeVent to allow the chaining of listeners as follows:
 
![image](https://user-images.githubusercontent.com/1453985/193402792-7e808de6-d649-4766-bcda-a29448fe8571.png)
 
These patterns allow for a more maintainable standardized system - wholly decoupled from its event source and capable of passing forward an event as an interceptor or completing a task and forwarding.
